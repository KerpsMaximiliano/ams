...
import { IKeycloackResponse } from '../models/keyCloackResponse.interface';
...
export class AuthService {
  ...
  public loginKeycloak(username: string, password: string) {
    ...
    return this.http
      .post<any>(
        `${this.environmentService.baseUrlKeycloak}/auth/realms/sancorsalud/protocol/openid-connect/token`,
        body.toString(),
        { headers: headers }
      )
      .pipe(
        map((res: IKeycloackResponse) => {
